...
documents and enriching it with ICD-11 codes using the ICD-11 API.
"""

from textwrap import dedent

from medminer.workflows.base.workflow import BaseExtractionWorkflow
from medminer.workflows.procedure.node import ICDProcedureLookup
...
class ProcedureExtractionWorkflow(BaseExtractionWorkflow[ProcedureState]):
    ...
    with standardized ICD-11 codes via the ICD-11 API.

    Attributes:
        prompt: The LLM prompt instructing how to extract procedure information.
        process_nodes: Tuple of processing nodes (ICDProcedureLookup) to enrich the data.
    """
    prompt = dedent("""\
